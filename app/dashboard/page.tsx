...
export default function Dashboard() {
  ...
  const [pauseTxHash, setPauseTxHash] = useState<string | null>(null);
  ...
  const handleManualPause = async () => {
    if (confirm("Are you sure you want to trigger a MANUAL EMERGENCY PAUSE? This will halt all bridge operations.")) {
      try {
        const result = await pauseBridge();
        if (result.ok && result.transactionHash) {
          setPauseTxHash(result.transactionHash);
        }
      } catch {
        alert("Failed to trigger manual pause. Is the contract deployed?");
      }
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center gap-6">
        <Spinner size="lg" />
        <p className="font-mono text-[#00E5FF] tracking-widest text-sm animate-pulse">ESTABLISHING SECURE CONNECTION...</p>
      </div>
    );
  }

  return (
    <div className="relative pb-12 px-4 sm:px-6 lg:px-8 font-sans mt-4">
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-stretch">
          <div className="flex-1 bg-slate-950/60 backdrop-blur-xl border border-slate-800 rounded-2xl p-6 flex flex-col justify-between">
            <div>
              ...
              <WorkflowActionCard 
                action={status?.ai?.recommendation} 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-5">
              ...
              <DashboardCard title="Locked Amount" value={`${status?.targetLocked || "0"} ETH`} subtitle="Cross-chain locked" status={status?.riskRatio && status.riskRatio > 60 ? "warning" : "info"} />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <FlagIndicator label="Governance" ok={!status?.governanceCompromised} trueText="COMPROMISED" falseText="Secure" />
              <FlagIndicator label="Proof Integrity" ok={!status?.failedProof} trueText="FORGED" falseText="Valid" />
              <FlagIndicator label="Circuit Breaker" ok={!status?.isPaused} trueText="TRIGGERED" falseText="Standby" />
              <FlagIndicator label="AI Engine" ok={!!status?.ai} trueText="Offline" falseText="Online" alwaysGreen />
            </div>

            {/* Pause Transaction Banner */}
            {(pauseTxHash || status?.lastTransactionHash) && (
              <div className="bg-red-950/30 border border-red-500/30 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-red-400 uppercase tracking-widest font-bold mb-1">Circuit Breaker Transaction</p>
                  <p className="text-xs text-slate-300 font-mono truncate max-w-[400px]">{pauseTxHash || status?.lastTransactionHash}</p>
                </div>
                <span className="badge badge-danger text-[10px]">ON-CHAIN</span>
              </div>
            )}
...
function FlagIndicator({
  ...
  const isGood = alwaysGreen ? ok : ok;
  return (
    <div className="bg-black/40 border border-slate-800/50 rounded-xl p-3 flex items-center gap-3">
      <div className={`h-2 w-2 rounded-full flex-shrink-0 ${isGood ? "bg-emerald-500 shadow-[0_0_6px_#10b981]" : "bg-red-500 shadow-[0_0_6px_#ef4444] animate-pulse"}`} />
      <div className="min-w-0">
        <p className="text-[9px] text-slate-500 uppercase tracking-widest">{label}</p>
        <p className={`text-xs font-bold truncate ${isGood ? "text-emerald-400" : "text-red-400"}`}>
          {isGood ? falseText : trueText}
        </p>
      </div>
    </div>
  );
