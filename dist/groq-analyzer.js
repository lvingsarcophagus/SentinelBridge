...
// ============ Prompt Construction ============
function buildAnalysisPrompt(riskScores, bridgeState) {
...
- Unauthorized Governance Event: ${bridgeState.unauthorizedGovernanceEvent ? "DETECTED" : "None"}
- Proof Verification Failure: ${bridgeState.failedProofVerification ? "DETECTED" : "None"}
 ...
 */
function analyzeWithGroq(runtime, riskScores, bridgeState, groqApiKey) {
    const getFallbackAnalysis = (scores) => ({
    ...
    const request = {
        ...
        body: JSON.stringify({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an AI Sentinel determining cross-chain bridge risk based on heuristic metrics. Analyze the provided data and respond ONLY in valid JSON.',
                },
                {
                    role: 'user',
                    content: prompt,
    ...
    const httpResponse = new cre_sdk_1.HTTPClient().sendRequest(runtime, request).result();
    ...
    try {
        ...
        const bodyStr = typeof httpResponse.body === 'string'
            ? httpResponse.body
            : Buffer.from(httpResponse.body).toString('utf-8');
        ...
        runtime.log(`[AI_RESULT] Assessment:\n` +
...
//# sourceMappingURL=groq-analyzer.js.map